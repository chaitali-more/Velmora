...
import BMRResult from "@/components/bmr/BMRResult";
import UnitToggle from "@/components/bmr/UnitToggle";
import { bmrFaqs } from "@/data/calculator-faqs";
...
export default function BMRPage() {
  ...
  const shareUrl = buildShareUrl("/bmr", {
    u: unit,
    age: values.age,
      ...
      <div className="relative mx-auto w-full max-w-7xl p-0 dark:rounded-[2rem] dark:border dark:border-white/10 dark:bg-slate-900/55 dark:p-3 sm:dark:p-4 md:dark:p-6">
        <div className="grid gap-4 lg:grid-cols-[1fr_1.08fr] lg:gap-5">
          <section className="rounded-3xl border border-slate-200/80 bg-white p-5 sm:p-6 dark:border-white/10 dark:bg-slate-900/65 dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.03),0_16px_40px_rgba(0,0,0,0.5)]">
            ...
            <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-900 dark:text-slate-100">BMR Calculator</h1>
            ...
            <div className="mt-6 sm:mt-7">
              ...
              <BMRForm values={values} errors={errors} unit={unit} onChange={handleChange} />
            ...
            {isCalculating ? (
              <div className="relative z-10 flex h-full min-h-[280px] flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300/80 bg-white/60 px-5 text-center sm:min-h-[340px] sm:px-6 dark:border-slate-700 dark:bg-slate-900/50">
                ...
                <p className="mt-3 max-w-sm text-sm leading-relaxed text-slate-600 dark:text-slate-300">
                  Applying your selected formula and activity factor now.
                </p>
              </div>
            ) : !result ? (
              <ResultPlaceholder description="Add age, gender, height, weight, and optional activity level. Your BMR and TDEE estimate will appear here." />
            ) : (
              <div className="relative z-10">
                <BMRResult
                  ...
                  actions={
                    <ResultActions
                      title="BMR Calculator Result"
                      shareUrl={shareUrl}
                      filename="velmora-bmr-result"
                      summaryLines={[
                        `BMR: ${result.bmr} kcal/day`,
                        `TDEE: ${result.tdee ?? "--"} kcal/day`,
                        `Activity: ${values.activityLevel || "Not selected"}`,
                      ]}
                    />
                  }
                />
