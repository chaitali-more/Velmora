...
import BodyFatResult from "@/components/body-fat/BodyFatResult";
import UnitToggle from "@/components/body-fat/UnitToggle";
import { bodyFatFaqs } from "@/data/calculator-faqs";
...
export default function BodyFatPage() {
  ...
  const shareUrl = buildShareUrl("/body-fat", {
    u: unit,
      ...
      <div className="relative mx-auto w-full max-w-7xl p-0 dark:rounded-[2rem] dark:border dark:border-white/10 dark:bg-slate-900/55 dark:p-3 sm:dark:p-4 md:dark:p-6">
        <div className="grid gap-4 lg:grid-cols-[1fr_1.08fr] lg:gap-5">
          <section className="rounded-3xl border border-slate-200/80 bg-white p-5 sm:p-6 dark:border-white/10 dark:bg-slate-900/65 dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.03),0_16px_40px_rgba(0,0,0,0.5)]">
            ...
            <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-900 dark:text-slate-100">Body Fat Calculator</h1>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
             Estimate your body fat percentage and understand your body composition instantly.
            </p>

            <div className="mt-6 sm:mt-7">
              <UnitToggle unit={unit} onChange={changeUnit} />
              <BodyFatForm values={values} errors={errors} unit={unit} onChange={handleChange} />

              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <button
                  onClick={calculate}
                  disabled={isCalculating}
                  className="rounded-2xl bg-gradient-to-br from-cyan-400 via-violet-500 to-fuchsia-500 py-4 text-base font-bold text-white shadow-[0_14px_28px_rgba(167,139,250,0.45)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_36px_rgba(167,139,250,0.55)] disabled:cursor-not-allowed disabled:opacity-70"
                >
                  {isCalculating ? "Calculating..." : "Calculate"}
                </button>
                <button
                  onClick={reset}
                  className="rounded-2xl border border-white/70 bg-white/85 py-4 text-base font-bold text-slate-700 shadow-[inset_0_1px_0_rgba(255,255,255,0.95),0_12px_22px_rgba(15,23,42,0.12)] transition hover:-translate-y-0.5 hover:text-slate-900 dark:border-white/15 dark:bg-slate-800/90 dark:text-slate-200 dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_12px_22px_rgba(0,0,0,0.4)] dark:hover:text-white"
                >
                  Reset
                </button>
              </div>
          ...
          <section className="relative overflow-hidden rounded-3xl border border-slate-200/80 bg-white p-5 sm:p-6 dark:border-white/10 dark:bg-slate-900/70 dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.03),0_20px_52px_rgba(0,0,0,0.55)]">
            ...
            {isCalculating ? (
              <div className="relative z-10 flex h-full min-h-[280px] flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300/80 bg-white/60 px-5 text-center sm:min-h-[340px] sm:px-6 dark:border-slate-700 dark:bg-slate-900/50">
                ...
                <p className="mt-3 max-w-sm text-sm leading-relaxed text-slate-600 dark:text-slate-300">
                  Applying the U.S. Navy method to your measurements now.
                </p>
              </div>
            ) : !result ? (
              <ResultPlaceholder description="Fill in your measurements and press Calculate. Your body fat percentage and derived metrics will appear here." />
            ) : (
                ...
                <BodyFatResult
                  ...
                  actions={
                    <ResultActions
                      ...
                      shareUrl={shareUrl}
                      filename="velmora-body-fat-result"
                      summaryLines={[
                        `Body fat: ${result.bodyFatPercentage.toFixed(1)}%`,
                        `Fat mass: ${result.fatMassKg.toFixed(1)} kg`,
                        `Lean mass: ${result.leanMassKg.toFixed(1)} kg`,
                      ]}
                    />
                  }
                />
